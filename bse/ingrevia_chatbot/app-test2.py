...
def load_css():
    ...
    <style>
        ...
        /* 타입 선택 버튼들만 아이보리 스타일 (피부타입, 피부고민, 제품카테고리) */
        .stButton > button[data-testid*="skin_"],
        .stButton > button[data-testid*="concern_"],
        .stButton > button[data-testid*="cat_"] {{
            background: linear-gradient(135deg, {COLORS['ivory']} 0%, #f5f7f2 100%);
            color: {COLORS['primary']};
            border: 2px solid {COLORS['border']};
            box-shadow: 0 3px 10px rgba(248, 249, 245, 0.3);
        }}
        
        .stButton > button[data-testid*="skin_"]:hover,
        .stButton > button[data-testid*="concern_"]:hover,
        .stButton > button[data-testid*="cat_"]:hover {{
            background: linear-gradient(135deg, #f0f2ed 0%, #eaede6 100%);
            color: {COLORS['button_hover']};
            border-color: {COLORS['accent']};
            transform: translateY(-2px);
            box-shadow: 0 6px 18px rgba(240, 242, 237, 0.4);
        }}
        
        .stButton > button[data-testid*="skin_"]:active,
        .stButton > button[data-testid*="concern_"]:active,
        .stButton > button[data-testid*="cat_"]:active {{
            background: linear-gradient(135deg, #e8ebe3 0%, #e2e6dd 100%);
            transform: translateY(0);
            box-shadow: 0 2px 8px rgba(232, 235, 227, 0.3);
        }}
