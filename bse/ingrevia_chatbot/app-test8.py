...
def analyze_ingredients_with_search(product_name: str, skin_type: str, skin_concerns: List[str], ingredients_list: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 종합 성분 분석 프롬프트 (더 정확하게)
        analysis_prompt = f"""
        당신은 전문 화장품 성분 분석가입니다. 다음 정보를 바탕으로 정확한 성분 분석을 해주세요.
        ...
        [분석 지침]
        1. 전성분 목록에서 {skin_type} 피부에 도움이 되는 **핵심 효능 성분** 3-5개를 찾아주세요.
        2. {skin_type} 피부나 {', '.join(skin_concerns)} 고민에 **주의해야 할 성분**을 꼼꼼히 찾아보세요:
           - 알코올류 (Alcohol, Denatured Alcohol)
           - 인공향료 (Fragrance, Parfum) 
           - 방부제 (Parabens, MIT, CMIT)
           - 계면활성제 (SLS, SLES)
           - 에센셜 오일류 (민감성 피부시)
           - 각질 제거 성분 (AHA, BHA - 과도할 경우)
           - 기타 자극 성분들
        3. 이 제품을 사용자에게 추천하는 **구체적인 이유**를 2문장으로 설명해주세요.

        [출력 형식] 반드시 JSON 형식으로 응답:
        {{
          "beneficial_ingredients": ["성분1 (효능설명)", "성분2 (효능설명)", ...],
          "caution_ingredients": ["성분A (주의이유)", "성분B (주의이유)", ...],
          "reason": "구체적인 추천 이유..."
        }}
        """
        ...
        return {
            "beneficial_ingredients": ["하이알루론산 (보습)", "세라마이드 (장벽강화)", "나이아신아마이드 (진정)"], 
            "caution_ingredients": ["분석 중 오류로 확인 필요"],
