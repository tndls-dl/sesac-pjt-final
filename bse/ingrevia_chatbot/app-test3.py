...
def load_css():
    ...
    <style>
        ...
        /* 선택 단계 버튼들을 아이보리 스타일로 오버라이드 */
        .stButton > button:not([kind="primary"]):not(.main-action-btn) {{
            background: linear-gradient(135deg, {COLORS['ivory']} 0%, #f5f7f2 100%) !important;
            color: {COLORS['primary']} !important;
            border: 2px solid {COLORS['border']} !important;
            box-shadow: 0 3px 10px rgba(248, 249, 245, 0.3) !important;
        }}
        
        .stButton > button:not([kind="primary"]):not(.main-action-btn):hover {{
            background: linear-gradient(135deg, #f0f2ed 0%, #eaede6 100%) !important;
            color: {COLORS['button_hover']} !important;
            border-color: {COLORS['accent']} !important;
            transform: translateY(-2px) !important;
            box-shadow: 0 6px 18px rgba(240, 242, 237, 0.4) !important;
        }}
        
        .stButton > button:not([kind="primary"]):not(.main-action-btn):active {{
            background: linear-gradient(135deg, #e8ebe3 0%, #e2e6dd 100%) !important;
            transform: translateY(0) !important;
            box-shadow: 0 2px 8px rgba(232, 235, 227, 0.3) !important;
        }}
