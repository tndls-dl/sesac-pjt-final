...
def main():
    ...
    if not all(profile.values()):
        ...
        # 분석이 완료된 경우, 결과를 계속 표시
        if st.session_state.get("analysis_done", False) and st.session_state.get("recommendations"):
            st.success(f"✨ **분석 완료!** 사용자님께 최적화된 **TOP 3 {profile['product_category']}** 제품을 추천합니다.")
            for i, rec in enumerate(st.session_state.recommendations, 1):
                render_product_card(rec, i)
        elif not st.session_state.get("analysis_done", False):
            with st.chat_message("assistant", avatar="🌿"):
