...
def render_product_card(rec: Dict[str, Any], rank: int):
    ...
    with st.container():
        ...
        <div style="background: white; border-radius: 15px; padding: 2rem; margin: 1rem 0; 
            ...
            <h3 style="color: {COLORS['primary']}; margin-bottom: 0.8rem;">{rank_emoji} TOP {rank}</h3>
            <h2 style="margin-bottom: 0.5rem;">
                <a href="{rec.get('링크', '#')}" target="_blank" 
                   style="color: {COLORS['primary']}; text-decoration: none; font-weight: 700;">
                   {rec.get('제품명', '이름 없음')}
                </a>
            </h2>
            <p style="color: {COLORS['text_soft']}; margin-bottom: 0.8rem;">
                <strong>🏢 브랜드:</strong> {rec.get('브랜드명', '브랜드 없음')}
            </p>
            <p style="color: {COLORS['text_soft']}; margin-bottom: 1.5rem;">
                <strong>💰 가격:</strong> {int(rec.get('가격', 0)):,}원 | 
                <strong>📏 용량:</strong> {rec.get('용량', '정보 없음')}
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # 핵심 효능 성분
        st.markdown(f"**✅ 이 제품의 핵심 효능 성분**")
        beneficial_ingredients = rec.get("beneficial_ingredients", [])
        if beneficial_ingredients:
            cols = st.columns(len(beneficial_ingredients))
            for i, ingredient in enumerate(beneficial_ingredients):
                with cols[i]:
                    st.success(ingredient)
        else:
            st.write("핵심 효능 성분을 분석 중입니다.")
        
        # 유의할 성분
        st.markdown(f"**⚠️ 내 피부에 유의할 성분**")
        caution_ingredients = rec.get("caution_ingredients", [])
        if caution_ingredients:
            cols = st.columns(len(caution_ingredients))
            for i, ingredient in enumerate(caution_ingredients):
                with cols[i]:
                    st.warning(ingredient)
        else:
            st.write("✅ 분석 결과, 특별히 유의할 성분은 발견되지 않았어요.")
...
def analyze_and_recommend(categories, profile, df):
    """분석을 수행하고 결과를 저장하는 함수"""
    for category in categories:
        # 분석 시작 메시지 즉시 표시
        analysis_msg = f"🔬 **{category}** 분석을 시작합니다...\n\n"
        if profile['skin_type'] and profile['skin_type'] != '일반':
...
def handle_chat_input(prompt: str):
    ...
    with st.spinner("AI가 사용자의 말을 이해하고 있어요..."):
        ...
        # 제품 종류가 파싱된 경우 응답만 추가하고 분석은 main에서 처리
        if parsed_info.get("product_categories"):
            response_text = "네, 알겠습니다!\n\n바로 분석을 시작할게요! 🔬"
            st.session_state.messages.append({"role": "assistant", "content": response_text})
            # 분석할 카테고리 저장
            st.session_state.pending_analysis = parsed_info["product_categories"]
        else:
...
def main():
    ...
    # 채팅 입력 처리
    if prompt := st.chat_input("예: 로션 추천해줘, 민감성 피부 크림 찾아줘"):
    ...
    if st.session_state.get('pending_analysis'):
        categories = st.session_state.pending_analysis
        analyze_and_recommend(categories, st.session_state.user_profile, df)
        # 분석 완료 후 대기 상태 해제
        st.session_state.pending_analysis = None
    ...
    if st.session_state.get('analysis_results'):
        for result in st.session_state.analysis_results:
            # 카테고리별 성공 메시지는 이미 messages에 있으므로 제품 카드만 표시
            for i, rec in enumerate(result['recommendations'], 1):
                render_product_card(rec, i)

    # 버튼 선택 UI - 채팅으로 입력하지 않은 경우에만 표시
    if not any('제품' in msg['content'] or '추천' in msg['content'] or '크림' in msg['content'] or '로션' in msg['content'] or '토너' in msg['content'] or '에센스' in msg['content'] or '앰플' in msg['content'] or '세럼' in msg['content'] or '선크림' in msg['content'] for msg in st.session_state.messages if msg['role'] == 'user'):
        
        # 피부 타입 선택
        if profile["skin_type"] is None:
            skin_types = ['민감성', '지성', '건성', '복합성', '아토피성', '중성', '해당없음/건너뛰기']
            cols = st.columns(3)
            for i, skin_type in enumerate(skin_types):
                if cols[i % 3].button(f"🫧 {skin_type}", key=f"skin_{skin_type}", use_container_width=True):
                    if skin_type == '해당없음/건너뛰기':
                        profile["skin_type"] = "일반"  # 일반으로 설정
                        user_msg = "피부 타입을 건너뛰겠습니다."
        ...
        # 피부 고민 선택 (피부타입이 설정된 경우)
        elif profile["skin_concerns"] is None:
            concerns_options = ['보습', '진정', '미백', '주름/탄력', '모공/피지', '트러블', '각질']
            emoji_map = {'보습': '💦', '진정': '🍃', '미백': '✨', '주름/탄력': '🌟', '모공/피지': '🔍', '트러블': '🩹', '각질': '🧽'}
            cols = st.columns(4)
            ...
            if col1.button("✅ 고민 선택 완료", type="primary", use_container_width=True, disabled=not st.session_state.selected_concerns):
            ...
            if col2.button("⏭️ 고민 선택 건너뛰기", use_container_width=True):
                profile["skin_concerns"] = []  # 빈 리스트로 설정
                user_msg = "피부 고민을 건너뛰겠습니다."
                ai_msg = "알겠습니다!\n\n어떤 **제품 종류**를 찾아드릴까요? (여러 개 선택 가능)"
                st.session_state.messages.append({"role": "user", "content": user_msg})
                st.session_state.messages.append({"role": "assistant", "content": ai_msg})
        ...
        # 제품 종류 선택
            ...
            categories = [('스킨/토너', '💧'), ('로션/에멀전', '🧴'), ('에센스/앰플/세럼', '✨'), ('크림', '🍶'), ('선크림/로션', '☀️')]
            cols = st.columns(3)
            ...
            if st.session_state.selected_categories:
                ...
                col1, col2 = st.columns(2)
                if col1.button("✅ 분석 시작하기", type="primary", use_container_width=True):
                    ...
                    # 분석 수행
                    analyze_and_recommend(profile["product_categories"], profile, df)
                
                if col2.button("🗑️ 선택 초기화", use_container_width=True):
                    st.session_state.selected_categories = []
    ...
    if st.session_state.get('show_reset_button', False):
        ...
        if col2.button("🔄 새 추천 시작하기", type="primary", use_container_width=True, key="main_reset"):
            st.session_state.clear()
            st.rerun()
    ...
    with st.sidebar:
        ...
        if profile.get('skin_type') and profile.get('skin_type') != '일반': 
